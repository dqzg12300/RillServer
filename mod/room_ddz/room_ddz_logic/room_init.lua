...
function game_hander.ongamestart(fsmself,event,form,to,msg)
	...
	if not RoomDDZ._players then
		ERROR("ddz room play is nil")
		return
	end
	local play_count=tablex.size(RoomDDZ._players)
...
function game_hander.onwaitallot(fsmself,event,form,to,msg)
	...
	RoomDDZ:broadcast({_cmd="game.deal",master=uids[uidIndex]})
	uidIndex=uidIndex+1
	if uidIndex>3 then
		uidIndex=1
	end
...
function RoomDDZ:initialize()
	...
	fsm=machine.create({
		initial="begin",
		events={
			{name="gamestart",from={"begin","become_master_wait"},to="enter_landlord"},				 --游戏开始
			{name="waitallot",from={"enter_landlord","become_master_wait"},to="become_master_wait"},	 --等待选地主
